...
export function detectPinch(handKeypoints, threshold = 0.08) {
  ...
  // In webcam monocular tracking, dz contains significant depth noise.
  // Pinch is confirmed if 3D distance is below threshold OR 2D screen distance is tight (< 0.068)
  const isPinching = distance3D < threshold || distance2D < (threshold * 0.85);

  return {
    isPinching,
    distance: Number(distance3D.toFixed(4)),
    distance2D: Number(distance2D.toFixed(4))
  };
}
...
export function mapToArenaSpace(input, config = {}) {
  ...
  if (input.middleTip) {
    const rawX = typeof input.middleTip.x === 'number' ? input.middleTip.x : 0.5;
    const rawY = typeof input.middleTip.y === 'number' ? input.middleTip.y : 0.5;
    mapped.cameraMiddleTip = {
      x: mirror ? 1.0 - rawX : rawX,
      y: rawY,
      z: typeof input.middleTip.z === 'number' ? input.middleTip.z : 0
    };
  }
  if (input.ringTip) {
    const rawX = typeof input.ringTip.x === 'number' ? input.ringTip.x : 0.5;
    const rawY = typeof input.ringTip.y === 'number' ? input.ringTip.y : 0.5;
    mapped.cameraRingTip = {
      x: mirror ? 1.0 - rawX : rawX,
      y: rawY,
      z: typeof input.ringTip.z === 'number' ? input.ringTip.z : 0
    };
  }
  if (input.pinkyTip) {
    const rawX = typeof input.pinkyTip.x === 'number' ? input.pinkyTip.x : 0.5;
    const rawY = typeof input.pinkyTip.y === 'number' ? input.pinkyTip.y : 0.5;
    mapped.cameraPinkyTip = {
      x: mirror ? 1.0 - rawX : rawX,
      y: rawY,
...
export class HandTracker {
  ...
  constructor(options = {}) {
    ...
    this.adaptiveSmoothing = Boolean(options.adaptiveSmoothing);
  ...
  detectForVideo(videoElement, timestamp = 0) {
    ...
    if (videoElement && typeof videoElement.readyState === 'number' && videoElement.readyState < 2) {
      return this.currentHands;
    }

    // MediaPipe requires integer, strictly monotonically increasing timestamps
    const now = Math.round(typeof timestamp === 'number' && timestamp > 0 ? timestamp : performance.now());
  ...
  applySmoothing(rawHandStates, alpha = this.alpha, timestamp = 0) {
    ...
    for (const raw of sortedRaw) {
      ...
      if (!prev) {
        ...
        let adaptiveAlpha = effectiveAlpha;
        if (this.adaptiveSmoothing) {
          const dx = raw.indexTip.x - prev.indexTip.x;
          const dy = raw.indexTip.y - prev.indexTip.y;
          const dz = (raw.indexTip.z || 0) - (prev.indexTip.z || 0);
          const distMoved = Math.sqrt(dx * dx + dy * dy + dz * dz);
          if (distMoved > 0.005) {
            adaptiveAlpha = Math.min(0.96, Math.max(effectiveAlpha, effectiveAlpha + (distMoved / 0.04) * (1.0 - effectiveAlpha)));
          }
        }

        const smoothedIndex = applyEmaSmoothing(prev.indexTip, raw.indexTip, adaptiveAlpha);
        const smoothedThumb = applyEmaSmoothing(prev.thumbTip, raw.thumbTip, adaptiveAlpha);
        const smoothedMiddle = applyEmaSmoothing(prev.middleTip, raw.middleTip, adaptiveAlpha);
        const smoothedRing = (raw.ringTip && prev.ringTip)
          ? applyEmaSmoothing(prev.ringTip, raw.ringTip, adaptiveAlpha)
          : (raw.ringTip || null);
        const smoothedPinky = (raw.pinkyTip && prev.pinkyTip)
          ? applyEmaSmoothing(prev.pinkyTip, raw.pinkyTip, adaptiveAlpha)
          : (raw.pinkyTip || null);
        const smoothedWrist = applyEmaSmoothing(prev.wrist, raw.wrist, adaptiveAlpha);
        const smoothedCamIndex = (raw.cameraIndexTip && prev.cameraIndexTip)
          ? applyEmaSmoothing(prev.cameraIndexTip, raw.cameraIndexTip, adaptiveAlpha)
          : (raw.cameraIndexTip || null);
        const smoothedCamThumb = (raw.cameraThumbTip && prev.cameraThumbTip)
          ? applyEmaSmoothing(prev.cameraThumbTip, raw.cameraThumbTip, adaptiveAlpha)
          : (raw.cameraThumbTip || null);
        const smoothedCamMiddle = (raw.cameraMiddleTip && prev.cameraMiddleTip)
          ? applyEmaSmoothing(prev.cameraMiddleTip, raw.cameraMiddleTip, adaptiveAlpha)
          : (raw.cameraMiddleTip || null);
        const smoothedCamRing = (raw.cameraRingTip && prev.cameraRingTip)
          ? applyEmaSmoothing(prev.cameraRingTip, raw.cameraRingTip, adaptiveAlpha)
          : (raw.cameraRingTip || null);
        ...
        // Thumb downward speed for Chord Slam gesture trigger
        const thumbVy = (dt > 0 && smoothedThumb && prev.thumbTip)
          ? (smoothedThumb.y - prev.thumbTip.y) / dt
          : 0;

        // Re-evaluate pinch using smoothed camera coordinates with hysteresis for rock-solid stability
        const isPrevPinch = prev ? Boolean(prev.isPinching) : false;
        const pinchThresh = isPrevPinch ? 0.095 : 0.075;
        const pinchEval = (smoothedCamIndex && smoothedCamThumb)
          ? detectPinch({ indexTip: smoothedCamIndex, thumbTip: smoothedCamThumb }, pinchThresh)
        ...
        smoothedHand = {
          ...
          ringTip: smoothedRing,
          pinkyTip: smoothedPinky,
          wrist: smoothedWrist,
          ...
          thumbVelocityY: Number(thumbVy.toFixed(4)),
