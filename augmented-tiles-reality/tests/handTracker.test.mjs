...
test('HandTracker extracts and tracks all 5 fingertips with adaptive low-latency smoothing', () => {
  const tracker = new HandTracker({ alpha: 0.5, adaptiveSmoothing: true, mirror: false });
  assert.equal(tracker.adaptiveSmoothing, true);
  ...
  const keypoints = extractKeypoints(mockLandmarks);
  assert.ok(keypoints.thumbTip);
  assert.ok(keypoints.indexTip);
  assert.ok(keypoints.middleTip);
  assert.ok(keypoints.ringTip);
  assert.ok(keypoints.pinkyTip);
});
