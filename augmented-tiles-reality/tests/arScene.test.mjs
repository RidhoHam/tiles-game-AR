...
test('ARScene setViewMode switches between tiles and roll (Real Piano) modes', () => {
  const scene = new ARScene({ laneCount: 8, arenaWidth: 0.8 });
  assert.equal(scene.viewMode, 'tiles');

  scene.setViewMode('roll');
  assert.equal(scene.viewMode, 'roll');

  scene.setViewMode('tiles');
  assert.equal(scene.viewMode, 'tiles');
});
